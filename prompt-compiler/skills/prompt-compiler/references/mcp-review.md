...
review first. When the tool is available, the render_prompt_review renderer
call is required; call it exactly once with the complete review before emitting
any full text fallback or assistant-authored review. The renderer response is
the presented review and is presentation only, not execution; it occurs before
the host waits for approval. The MCP server only
...
continue the canonical Milestone 2 action protocol. After the
render_prompt_review call returns the host must stop immediately, or stop
immediately after fallback text is emitted, and wait for the user's next
message. Do not send a review payload
