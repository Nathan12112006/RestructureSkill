...
import type { PromptReview } from './schema.js';
...
export function renderTextFallback(review: PromptReview): string {
  const assumptions = [
  ...
  const applied = review.applied_user_instructions.length
    ? review.applied_user_instructions.map((item) => `- ${item.text} — Source: ${item.source}`).join('\n')
    : '(none)';
